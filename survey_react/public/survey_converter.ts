...
function convertToSurveyJS(inputData: InputSurvey): SurveyJSOutput {
      ...
      try {
        ...
        if (question.options === null) {
          ...
          if (question.options && question.options.length > 0) {
            const choiceArray: SurveyJSChoice[] = question.options.map(option => ({
              value: option,
              text: option
            }));
...
// Usage function to convert and save
function convertAndSaveJson(inputJson: string, outputFilePath: string): void {
  try {
    const inputData: InputSurvey = JSON.parse(inputJson);
    const surveyJSData = convertToSurveyJS(inputData);
    
    // In a browser environment, you might use:
    // localStorage.setItem('surveyJSData', JSON.stringify(surveyJSData, null, 2));
    
    // In Node.js, you would use:
    // const fs = require('fs');
    // fs.writeFileSync(outputFilePath, JSON.stringify(surveyJSData, null, 2));
